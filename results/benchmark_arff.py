...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname("__file__"), '..')))
from pyod.models.copod import COPOD as COD
...
for j in range(len(arff_list)):
    ...
    for i in range(n_ite):
        ...
        classifiers = {'Angle-based Outlier Detector (ABOD)': ABOD(
            ...
            'BAZUOD': BAZUOD(contamination=outliers_fraction)
        }
        classifiers_indices = {
    ...
    time_df.to_csv('time_arff.csv', index=False, float_format='%.3f')
    roc_df.to_csv('roc_arff.csv', index=False, float_format='%.3f')
    prn_df.to_csv('prc_arff.csv', index=False, float_format='%.3f')
    ap_df.to_csv('ap_arff.csv', index=False, float_format='%.3f')
